...
当前 `development` 适配 **SDK/API 0.4.0**，API 地址为 `https://api.heixi.com`。线上 0.4.0 资源已重新发布并核对；本示例已完成独立自动回归和 SDK/API 各一次真实主图生成验收，详细范围见发布记录。接入方仍需按自己的商户配置与授权验收。 旧版请使用 `release-0.3.0`。分支、验收与回退见[开发与发布记录](docs/development-release.md)。没有本地平台接入模式、商户数据空间配置或多套账本；不需要构建、导入或启动相邻项目。
