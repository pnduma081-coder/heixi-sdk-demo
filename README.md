...
当前 `development` 适配 **SDK/API 0.4.0**，API 地址为 `https://api.heixi.com`。线上 0.4.0 资源已重新发布并核对；本示例仍在开发分支，真实商户接入需完成验收。 旧版请使用 `release-0.3.0`。分支、验收与回退见[开发与发布记录](docs/development-release.md)。没有本地平台接入模式、商户数据空间配置或多套账本；不需要构建、导入或启动相邻项目。
